...
####==================================####
     ...
     for orf_file in "${VIRUS_INPUT}"/*_orfs.fasta; do
        ...
        for single_orf in "${orf_split_dir}"/*_orf_*.fasta; do
            ...
            # Extract ORF number from filename
            orf_num=$(basename "$single_orf" .fasta | rev | cut -d'_' -f1 | rev)

            echo "  BLASTing ORF ${orf_num}..."

            blast_out="${VIRUS_OUTPUT}/${sample_name}_orf${orf_num}_blast.tsv"
            ...
            if [ $? -eq 0 ]; then
                echo "    BLAST complete: $(basename "$blast_out")"

                # Parse top hit for CSV
