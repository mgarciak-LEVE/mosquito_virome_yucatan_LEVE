#!/bin/bash
# Single FastQC task for job array post-trimming
...
INPUT_FASTQC="${PROJECT_SCRATCH}/results/trimmed"
OUTPUT_BASE="${PROJECT_SCRATCH}/results"
OUTPUT_FASTQC="${OUTPUT_BASE}/trimmed_qc/fastqc"
...
while IFS= read -r line; do
    files+=("$line")
done < <(find . -name "*.fastq" -o -name "*.fastq.gz" | sort)
...
tg_send "Post-trimming FastQC: ${file_name} (${LSB_JOBINDEX}/${#files[@]})" 2>/dev/null || true
...
if apptainer exec \
    ...
    echo "Post-trimming FastQC FAILED for ${file_name}"
    tg_send "Post-trimming FastQC: ${file_name} FAILED" 2>/dev/null || true
    exit 1
fi
